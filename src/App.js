...
export default function App() {
  ...
  const Tab = ({ id, label, icon }) => (
    <button onClick={() => setActiveTab(id)} style={{ background: activeTab === id ? ACCENT : 'transparent', color: activeTab === id ? '#fff' : MUTED, border: 'none', borderRadius: 8, padding: '8px 16px', fontWeight: 600, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: SURFACE, fontFamily: "'Inter', system-ui, sans-serif", color: TEXT }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 16px' }}>
        {toast && (
          <div style={{ position: 'fixed', top: 16, right: 16, zIndex: 1000, background: toast.type === 'error' ? DANGER : SUCCESS, color: '#fff', borderRadius: 10, padding: '12px 18px', fontWeight: 600, fontSize: 14, boxShadow: '0 4px 20px #0003' }}>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
          <div style={{ flex: '0 0 auto', position: 'relative' }}>
            <label htmlFor="xl-input" style={{ border: `2px dashed ${dragOver ? ACCENT : BORDER}`, borderRadius: 12, background: dragOver ? ACCENT_LIGHT : CARD, padding: '12px 18px', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}
              onDrop={(e) => { e.preventDefault(); setDragOver(false); handleFile(e.dataTransfer.files[0]) }}
              onDragOver={(e) => { e.preventDefault(); setDragOver(true) }}
              onDragLeave={() => setDragOver(false)}>
              <span style={{ fontSize: 20 }}>{loading ? '⏳' : '📂'}</span>
              <div>
                <div style={{ fontWeight: 700, fontSize: 13 }}>{clients.length > 0 ? 'Planilha importada ✓' : 'Importar Planilha'}</div>
                <div style={{ fontSize: 11, color: MUTED }}>{clients.length > 0 ? `${clients.length} clientes • ${routes.length} rotas` : 'Toque aqui ou use Colar Dados'}</div>
              </div>
            ...
            <button onClick={() => setShowPaste(v => !v)} style={{ marginTop: 5, width: '100%', background: 'none', border: `1px solid ${BORDER}`, borderRadius: 8, padding: '6px 10px', fontSize: 11, fontWeight: 600, color: ACCENT, cursor: 'pointer' }}>
              📋 {showPaste ? 'Fechar' : 'Colar Dados (celular)'}
            </button>
            {showPaste && (
              <div style={{ position: 'absolute', top: '110%', left: 0, zIndex: 200, width: 300, background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, boxShadow: '0 8px 32px #0003', padding: 14 }}>
                ...
                <textarea rows={7} placeholder={"Cliente\tRota\tStatus\nJoão\tRota Norte\t\nMaria\tRota Sul\tinativo"} value={pasteText} onChange={e => setPasteText(e.target.value)} style={{ width: '100%', border: `1px solid ${BORDER}`, borderRadius: 8, padding: '8px', fontSize: 11, fontFamily: 'monospace', resize: 'vertical', boxSizing: 'border-box' }} />
          ...
          {routes.length > 0 && (
            <div style={{ flex: 1, minWidth: 180, background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ fontSize: 18 }}>🗺️</span>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700, fontSize: 12, marginBottom: 4 }}>Rota do Dia</div>
          ...
          {selectedRoute && (
            <div style={{ flex: '0 0 auto', background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ fontSize: 18 }}>🎯</span>
              <div>
                <div style={{ fontWeight: 700, fontSize: 12, marginBottom: 4 }}>Meta do Dia</div>
                {dailyGoal ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontWeight: 800, color: ACCENT, fontSize: 15 }}>{fmt(dailyGoal)}</span>
                    <button onClick={() => setDailyGoal('')} style={{ background: 'none', border: 'none', color: MUTED, cursor: 'pointer' }}>✏️</button>
                  </div>
                ) : (
                  <div style={{ display: 'flex', gap: 6 }}>
                    <input type="number" placeholder="R$ 0,00" value={goalInput} onChange={e => setGoalInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSetGoal()} style={{ width: 100, border: `1px solid ${BORDER}`, borderRadius: 7, padding: '5px 8px', fontSize: 13 }} />
                    <button onClick={handleSetGoal} style={{ background: ACCENT, color: '#fff', border: 'none', borderRadius: 7, padding: '5px 12px', fontWeight: 700, cursor: 'pointer', fontSize: 12 }}>Definir</button>
                  </div>
                )}
              </div>
            </div>
          ...
          <div>
            {!selectedRoute ? (
              <div style={{ textAlign: 'center', padding: '70px 20px', color: MUTED }}>
                ...
                <div style={{ fontWeight: 700, fontSize: 17, color: TEXT }}>Selecione uma rota para começar</div>
              </div>
            ) : (
              <>
                ...
                {dailyGoal > 0 && (
                  <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '16px 20px', marginBottom: 16 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                      <span style={{ fontWeight: 700, fontSize: 14 }}>🎯 Progresso da Meta</span>
                      <span style={{ fontWeight: 800, color: goalProgress >= 100 ? SUCCESS : ACCENT }}>{goalProgress.toFixed(1)}%</span>
                    </div>
                    <div style={{ background: SURFACE, borderRadius: 99, height: 10, overflow: 'hidden' }}>
                      <div style={{ width: `${goalProgress}%`, height: '100%', background: goalProgress >= 100 ? SUCCESS : ACCENT, borderRadius: 99, transition: 'width 0.4s' }} />
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6, fontSize: 12, color: MUTED }}>
                      <span>{fmt(totalSold)} vendidos</span>
                      <span>Faltam {fmt(Math.max(dailyGoal - totalSold, 0))}</span>
                    </div>
                  </div>
                )}

                {inactiveSoldClients.length > 0 && (
                  <div style={{ background: '#FFF7ED', border: `1.5px solid ${WARNING}55`, borderRadius: 14, padding: '16px 20px', marginBottom: 16 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                      <span>⚠️</span>
                      <span style={{ fontWeight: 700, fontSize: 14, color: WARNING }}>Clientes Inativos Atendidos</span>
                      <Badge color={WARNING}>{inactiveSoldClients.length}</Badge>
                      <div style={{ flex: 1 }} />
                      <span style={{ fontWeight: 800, color: WARNING }}>{fmt(inactiveSoldClients.reduce((a, s) => a + s.value, 0))}</span>
                    </div>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                      <tbody>
                        {inactiveSoldClients.map(s => (
                          <tr key={s.id}>
                            <td style={{ padding: '6px 8px', fontWeight: 600 }}>{s.client_name}</td>
                            <td style={{ padding: '6px 8px' }}><Badge color={WARNING}>{fmt(s.value)}</Badge></td>
                            <td style={{ padding: '6px 8px', color: MUTED, fontSize: 12 }}>{s.note || '—'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '18px 20px', marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 110 }}>
                      <label style={{ fontSize: 11, fontWeight: 600, color: MUTED, display: 'block', marginBottom: 4 }}>VALOR (R$)</label>
                      <input type="number" placeholder="0,00" value={saleValue} onChange={e => setSaleValue(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddSale()} style={{ width: '100%', border: `1px solid ${BORDER}`, borderRadius: 8, padding: '9px 10px', fontSize: 13, boxSizing: 'border-box' }} />
                    </div>
                    <div style={{ flex: 2, minWidth: 140 }}>
                      <label style={{ fontSize: 11, fontWeight: 600, color: MUTED, display: 'block', marginBottom: 4 }}>OBSERVAÇÃO</label>
                      <input type="text" placeholder="Opcional…" value={saleNote} onChange={e => setSaleNote(e.target.value)} style={{ width: '100%', border: `1px solid ${BORDER}`, borderRadius: 8, padding: '9px 10px', fontSize: 13, boxSizing: 'border-box' }} />
                    </div>
                ...
                {routeSales.length > 0 && (
                  <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, overflow: 'hidden' }}>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                      ...
                      <tbody>
                        {[...routeSales].reverse().map((s, i) => (
                          <tr key={s.id} style={{ borderBottom: `1px solid ${BORDER}`, background: i % 2 === 0 ? CARD : SURFACE }}>
          ...
          <div>
            {clients.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '70px 20px', color: MUTED }}>
                ...
                <div style={{ fontWeight: 700, fontSize: 17, color: TEXT }}>Nenhuma planilha importada</div>
              </div>
            ) : (
              <>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                    ...
                    <tbody>
                      {filteredClients.map((c, i) => (
                        <tr key={c.id} style={{ borderBottom: `1px solid ${BORDER}`, background: c.inactive ? '#FFF7ED' : (i % 2 === 0 ? CARD : SURFACE) }}>
        ...
        {activeTab === 'vendas' && (
          <div>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                <div style={{ flex: 2, minWidth: 180, position: 'relative' }}>
                  ...
                  {showSuggestions && clientSuggestions.length > 0 && (
                    <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 100, background: CARD, border: `1px solid ${BORDER}`, borderRadius: 8, boxShadow: '0 8px 24px #0002', marginTop: 4, overflow: 'hidden', maxHeight: 240, overflowY: 'auto' }}>
                      ...
                      {clientSuggestions.map(c => (
                        <div key={c.id} onMouseDown={() => { setTabSaleClient(c); setTabSaleClientInput(c.name); setShowSuggestions(false) }}
                ...
                <div style={{ flex: 1, minWidth: 110 }}>
                  <label style={{ fontSize: 11, fontWeight: 600, color: MUTED, display: 'block', marginBottom: 4 }}>VALOR (R$)</label>
                  <input type="number" placeholder="0,00" value={tabSaleValue} onChange={e => setTabSaleValue(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddTabSale()} style={{ width: '100%', border: `1px solid ${BORDER}`, borderRadius: 8, padding: '9px 10px', fontSize: 13, boxSizing: 'border-box' }} />
                </div>
                <div style={{ flex: 2, minWidth: 130 }}>
                  <label style={{ fontSize: 11, fontWeight: 600, color: MUTED, display: 'block', marginBottom: 4 }}>OBSERVAÇÃO</label>
                  <input type="text" placeholder="Opcional…" value={tabSaleNote} onChange={e => setTabSaleNote(e.target.value)} style={{ width: '100%', border: `1px solid ${BORDER}`, borderRadius: 8, padding: '9px 10px', fontSize: 13, boxSizing: 'border-box' }} />
                </div>
            ...
            {sales.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '48px 20px', color: MUTED, background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14 }}>
                <div style={{ fontSize: 36, marginBottom: 8 }}>📋</div>
                <div style={{ fontWeight: 700, color: TEXT }}>Nenhuma venda hoje</div>
              </div>
            ) : (
              <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
