...
const criarUsuario=async()=>{if(!novoEmail||!novaSenha||!novoNome||!novoEgestorCode){showToast('Preencha todos os campos','error');return}const perfilFinal=novoPerfil==='outro'?novoPerfilCustom.trim():novoPerfil;if(novoPerfil==='outro'&&!perfilFinal){showToast('Informe o nome do perfil.','error');return}setConfigLoading(true);try{const res=await fetch(`${ADMIN_API}?action=criar_usuario`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:novoEmail,password:novaSenha,name:novoNome,egestor_code:parseInt(novoEgestorCode),perfil:perfilFinal})});const data=await res.json();if(data.error){showToast('Erro: '+data.error,'error')}else{showToast('Usuário criado!');setNovoEmail('');setNovaSenha('');setNovoNome('');setNovoEgestorCode('');setNovoPerfil('vendedor');setNovoPerfilCustom('');await carregarConfig()}}catch(err){showToast('Erro ao criar usuário','error')}setConfigLoading(false)}
